...
def whitetest(session: dict):
    ...
    if pvalue > chi1:
        return "<h2>White Test</h2><p>chi({})={}***</p><p>99% heteroscedasticity</p>".format(
            len(coef), chi)
    if pvalue > chi2:
        return "<h2>White Test</h2><p>chi2({})={}**</p><p>95% heteroscedasticity</p>".format(
            len(coef), chi)
    if pvalue > chi3:
        return "<h2>White Test</h2><p>chi2({})={}*</p><p>90% heteroscedasticity</p>".format(
            len(coef), chi)
    return "<center><h2>White Test</h2><p>chi({})={}</p></center>".format(
        len(coef), chi)
